...
def check_card_stripe(card_number, exp_month, exp_year, cvc):
    try:
        ...
        if "insufficient_funds" in error:
            return "APPROVED", "✅ Live - Insufficient Funds"
        elif "card_declined" in error:
            return "DECLINED", "❌ Declined - Card Declined"
        elif "expired_card" in error:
            return "DECLINED", "❌ Declined - Card Expired"
        elif "incorrect_cvc" in error:
            return "DECLINED", "❌ Declined - Incorrect CVC"
        else:
            return "DECLINED", f"❌ Declined - {str(e)[:30]}"
...
def process_single_card(card_data):
    ...
    if not card_num.isdigit() or len(card_num) < 15:
        return "INVALID", "⚠️ Invalid - Wrong Format"
    
    if not luhn_check(card_num):
        return "INVALID", "⚠️ Invalid - Luhn Failed"
    
    status, message = check_card_stripe(card_num, exp_month, exp_year, cvc)
...
async def start(update: Update, context: CallbackContext):
    await update.message.reply_text(
        "🤖 **CC Checker Bot**\n\n"
        "📁 فایلێکی .txt باربکە کە کارتەکان به‌م فۆرماتەی تیا بێت:\n\n"
        "`5243021867159008|11|2028|100`\n"
        "`4242424242424242|12|2026|123`\n\n"
        "✅ Approved - کارت چالاکە\n"
        "❌ Declined - کارت ڕەتکراوە\n"
        "⚠️ Invalid - کارت نادروستە\n\n"
        "📊 دوای تەواوبوون، فایلی ئەنجامەکان بۆت دەنێردرێت.",
        parse_mode='Markdown'
    )
...
async def handle_file(update: Update, context: CallbackContext):
    ...
    if not CARDS_LIST:
        await update.message.reply_text("❌ هیچ کارتێک نەدۆزرایەوە لە فایلەکەدا!")
        return
    ...
    await update.message.reply_text(
        f"📁 فایل وەرگیرا!\n"
        f"📊 کۆی کارتەکان: {TOTAL_CARDS}\n"
        f"⏳ پشکنین دەستی پێکرد..."
    )
...
async def process_cards(update: Update, context: CallbackContext):
    ...
    for i, card_data in enumerate(CARDS_LIST):
        if not CHECKING:
            await msg.edit_text("🛑 پشکنین ڕاگرا!")
            break
        
        card_num = card_data['card']
        masked = f"{card_num[:4]}****{card_num[-4:]}"
        
        status, message = process_single_card(card_data)
        
        if status == "APPROVED":
            APPROVED_LIST.append(f"✅ {masked} → {message}")
        elif status == "DECLINED":
            DECLINED_LIST.append(f"❌ {masked} → {message}")
        else:
            INVALID_LIST.append(f"⚠️ {masked} → {message}")
        ...
        if i % 3 == 0 or i == TOTAL_CARDS - 1:
            try:
                await msg.edit_text(
                    ...
                    f"❌ Declined: {len(DECLINED_LIST)}\n"
                    f"⚠️ Invalid: {len(INVALID_LIST)}\n"
                    f"📊 پێشکەوتن: {i+1}/{TOTAL_CARDS}",
                    reply_markup=reply_markup
                )
            except:
                pass
    ...
    if CHECKING:
        ...
        if APPROVED_LIST:
            ...
            await update.message.reply_document(
                ...
                caption=f"✅ {len(APPROVED_LIST)} Approved Cards"
            )
        ...
        if DECLINED_LIST:
            ...
            await update.message.reply_document(
                ...
                caption=f"❌ {len(DECLINED_LIST)} Declined Cards"
            )
...
def main():
    ...
    print("🤖 بۆتەکە کارا بوو! چاوەڕوانی فایلەکانت دەکات...")
